...
$app->get('/company', function (Request $request, Response $response) {
    ...
    return $response->getBody()->write($company->toJson());
});
...
$app->get('/company/{id}', function (Request $request, Response $response, $args) {
    ...
    $id = $args['id'];
    try {
        $company = \App\Models\Company::with([])->find($id);
    } catch (\Exception $e) {
        // do task when error
...
$app->put('/company/{id}', function ($request, $response, $args) {
    $id = $args['id'];
    $data = $request->getParsedBody();
    print_r($data);
    try {
        ...
        $company->descr = $data['descr'] ?: $company->descr;
        $company->save();
    } catch (\Exception $e) {
        return $response->withStatus(404)->getBody()->write($e->getMessage());
    }
...
$app->delete('/company/{id}', function ($request, $response, $args) {
    $id = $args['id'];
    try {
        $company = \App\Models\Company::find($id);
        $company->delete();
